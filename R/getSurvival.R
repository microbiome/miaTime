    ...
    function(x, time.col, status.col, name = "survival", ...){
        ...
        args <- c(
            list(x = x, time.col = time.col, status.col = status.col,
                    name = name),
            list(...))
        res <- do.call(getSurvival, args) |> as.list()
    ...
    status.col, col.var, assay.type) 
{
    ...
    covar <- NULL
    if (!is.null(col.var))
        covar <- as.data.frame(colData(x)[, col.var, drop = FALSE])
    list(
        mat = mat,
        time = time,
        status = status,
...
# dispatcher to calculate survival
    ...
    coef.threshold = 0, ...) 
{
    if (penalized) {
        ...
        .fit_penalized_cox(
            mat, time, status, covar,
            alpha, nfolds, nvar, lambda, coef.threshold
        )
    } else {
        ## --- standard CoxPH ---
        .fit_standard_cox(mat, time, status, covar, coef.threshold)
    }
}

#' @importFrom survival Surv coxph
#' @importFrom glmnet cv.glmnet glmnet
# Penalized cox function
.fit_penalized_cox <- function(mat, time, status, covar,
    alpha, nfolds, nvar, lambda, coef.threshold) 
{
    y <- Surv(time, status)
    ...
    if (!is.null(covar)) {
        ...
        model0 <- coxph(y ~ ., data = df0)
        offset <- predict(model0, type = "lp")
    }
    ...
    ## --- lambda selection ---
    lambda.value <- .lambda_selector(cvfit, lambda, nvar)
    
    ## --- coefficients ---
    coefs <- as.vector(coef(cvfit, s = lambda.value))
    names(coefs) <- rownames(coef(cvfit, s = lambda.value))
    coef.res <- .filter_and_normalize_coefs(coefs, coef.threshold)
    
    ## --- predictions ---
    risk_scores <- as.numeric(
        predict(cvfit, mat, s = lambda.value, newoffset = offset)
    )
    ## --- apparent C-index ---
    Cindex_signature <- .compute_cindex(risk_scores, y, penalized = TRUE)
    ## --- CV stats at chosen lambda ---
    idrow <- max(which(cvfit$glmnet.fit$lambda >= lambda.value))
    
    list(
        coefficients = coef.res$coefficients,
        selected.features = coef.res$selected.features,
        risk_scores = risk_scores,
        cindex.apparent = Cindex_signature,
        cv.cindex.mean = cvfit$cvm[idrow],
        cv.cindex.sd = cvfit$cvsd[idrow],
        fit = cvfit
    )
}

# Standard cox function
.fit_standard_cox <- function(mat, time, status, covar, coef.threshold) {
    ## --- standard CoxPH ---
    y <- Surv(time, status)
    ...
    ## apparent C-index
    Cindex_signature <- .compute_cindex(risk_scores, y, fit, penalized = FALSE)
...
.filter_and_normalize_coefs <- function(coefs, threshold) {
    selected <- which(abs(coefs) > threshold)
...
.compute_cindex <- function(risk_scores, y, fit = NULL, penalized = TRUE) {
    if (length(risk_scores) == 0 || all(risk_scores == 0)) return(0.5)
    if (penalized) {
        glmnet::Cindex(pred = risk_scores, y)
    } else {
        summary(fit)$concordance[1]
    }
}

.lambda_selector <- function(cvfit, lambda, nvar = NULL) {
    ## --- nvar cutoff handling ---
    if (!is.null(nvar)) {
        valid <- which(cvfit$glmnet.fit$df <= nvar)
        if (length(valid) > 0) lambda <- cvfit$glmnet.fit$lambda[max(valid)]
    }
    ## --- lambda selection ---
