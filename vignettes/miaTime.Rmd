...
## Estimating divergence from a reference sample
...
se <- hitchip1006
reference_sample <- colnames(se[, which.min(colData(se)$time)])
...
# in a desired way:
fun <- function (se) {
  reference_sample <- rownames(colData(se)[which.min(colData(se)$time),])
